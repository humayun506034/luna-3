...
const updateTip = async (req: Request, res: Response) => {
  try {
    ...
    const updatedTip = await TipService.updateTip(req.params.id as string, parsedData);
    res.json(updatedTip);
  } catch (error: any) {
    res.status(400).json({ message: error.message });
  }
...
const deleteTip = async (req: Request, res: Response) => {
  try {
    ...
    await TipService.deleteTip(req.params.id as string);
...
const toggleSaveTip = async (req: Request, res: Response) => {
  try {
    ...
    const result = await TipService.toggleSave(userId, tipId as string);
    res.json({
...
const toggleLikeTip = async (req: Request, res: Response) => {
  try {
    ...
    const tipId = req.params.id;
    const result = await TipService.toggleLike(userId, tipId as string);
    res.json({
