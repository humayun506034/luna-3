...
const getAllExerciseVariantPerformedByUser = catchAsync(async (req, res) => {
  ...
  const result =
    await analysisService.getAllExerciseVariantPerformedByUser(user_id);
  res.status(200).json({
...
const runAnalysis = catchAsync(async (req, res) => {
  ...
  // Validate timeSpan
  const validTimeSpans: Array<
    '7_days' | '30_days' | '60_days' | '90_days' | 'yearly'
  > = ['7_days', '30_days', '60_days', '90_days', 'yearly'];
  ...
  if (filterParameter) {
    const validFilters: Array<
      'duration' | 'volume' | 'reps' | 'totalCaloryBurn' | 'set'
    > = ['duration', 'volume', 'reps', 'totalCaloryBurn', 'set'];
  ...
  const result = await analysisService.runAnalysis(
    ...
    filterParameter as
      ...
      | 'set',
    exerciseId
      ? (idConverter(exerciseId as string) as Types.ObjectId)
      : undefined
  );

  res
    .status(200)
    .json({ sucess: true, message: 'analysis done', data: result });
});
