...
const createUser = async (
  ...
  try {
    ...
    // Abort only if a transaction is still active
...
const createWorkoutSetup = async (user_id: Types.ObjectId, payload: any) => {
  ...
  if (
    ...
    throw new Error(
      'All workout setup fields, including sleepQuality, are required.'
    );
  }
  ...
  switch (payload.sleepQuality) {
    ...
    default:
      throw new Error(
        'Invalid sleep quality. Must be one of: excellent, great, normal, bad.'
      );
  }
  ...
  try {
    ...
    const workoutSetup = new WorkoutASetupModel({
      ...
      sleepQuality,
      mineralsGoal: payload?.mineralsGoal || 0,
      vitaminesGoal: payload?.vitaminesGoal || 0,
      proteinGoal: payload?.proteinGoal || 0,
      fatGoal: payload?.fatGoal || 0,
      carbsGoal: payload?.carbsGoal || 0,
      fatsGoal: payload?.fatsGoal || 0,
      fiberGoal: payload?.fiberGoal || 0,
      distanceGoal: payload?.distanceGoal || 0,
      setGoal: payload?.setGoal || 0,
      repsGoal: payload?.repsGoal || 0,
    });
    ...
    throw new Error(
      error.message ||
        'Failed to create workout setup due to an internal error.'
    );
  } finally {
    session.endSession();
    console.log('Session ended');
...
const updateWorkoutSetup = async (user_id: Types.ObjectId, payload: any) => {
  ...
  try {
    ...
    const workoutSetup = await WorkoutASetupModel.findOne({ user_id }).session(
      session
    );
    if (!workoutSetup) {
    ...
    throw new Error(
      error.message ||
        'Failed to update workout setup due to an internal error.'
    );
  } finally {
    session.endSession();
    console.log('Session ended');
...
const getWorkoutSetup = async (user_id: Types.ObjectId) => {
  ...
  const workoutSetup = await WorkoutASetupModel.findOne({ user_id }).populate(
    'user_id',
    'name email'
  );
