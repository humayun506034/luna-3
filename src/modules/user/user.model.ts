...
import { getCountryNameFromNumber } from '../../util/country';
...
const WorkoutASetupSchema = new Schema<TWorkoutASetup>({
  ...
  vitaminesGoal: { type: Number, required: false, default: 0 },
  mineralsGoal: { type: Number, required: false, default: 0 },
  sleepQuality: { type: SleepQualitySchema, required: true },
  distanceGoal: { type: Number, required: true, default: 0 },
  repsGoal: { type: Number, required: true, default: 0 },
  setGoal: { type: Number, required: true, default: 0 },
});
...
ProfileSchema.pre('save', async function (next) {
  ...
  try {
    const countryName = await getCountryNameFromNumber(this.phone!);

    // Set countryName if available
    if (countryName) {
    ...
    console.error('Error in pre-save hook for countryName:', error);
    next();
  }
});
...
export const WorkoutASetupModel = mongoose.model(
  'WorkoutASetup',
  WorkoutASetupSchema
);
